...
from modeling import PTuneForLAMA_freeze
# args
...
for epoch in range(args.epochs):
    ...
    for events, labels in train_loader:
        ...
        print('/n')
        print(loss)
torch.save(model, str(args.lr) + " " + str(args.epochs) + " new task3_ptuning_freeze_checkpoint.pkl")
...
for events, labels in dev_loader:
    with torch.no_grad():
        ...
        token_type_ids = token_type_ids.to(device)
        labels_ids = labels_ids.to(device)
        loss,logits,hit1 = model(input_ids, labels_ids,len(input_ids))
        pred_logits = logits
        bz = len(input_ids)
        for i in range(bz):
            ids = input_ids[i]
