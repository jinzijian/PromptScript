...
parser.add_argument("--mode", type=str, default="freeze", help="different settings")
...
if args.mode == 'finetune':
    include_model = torch.load("/home/zijian/Scripts_new/5e-05 20negative_size20 task1_finetune_checkpoint.pkl").to(device)
...
if args.mode == 'ptuning':
    include_model = torch.load("/home/zijian/Scripts_new/5e-05 10 task1_ptuning_checkpoint.pkl").to(device)
...
if args.mode == 'freeze':
    include_model = torch.load("/home/zijian/Scripts_new/5e-05 10negative100.0 newtest_task1_ptuning_nfreeze_checkpoint.pkl").to(device)
    start_model = torch.load("/home/zijian/Scripts_new/5e-05 20 new freeze_margin_task2_ptuning_checkpoint.pkl").to(device)
    order_model = torch.load("/home/zijian/Scripts_new/5e-05 20 new task3_ptuning_freeze_checkpoint.pkl").to(device)
...
for events, labels in test_loader:
    with torch.no_grad():
        ...
        for i in range(bz):
            ...
            include_res.append(pred_logit[masked_index][indexed_tokens_include])


print('finish evaluating')
print(str(len(include_res)))
include_res = np.array(include_res)
new_res = np.split(include_res, len(main_events))
include_results = []
for r in new_res:
    like  = r
    tmp = []
...
def get_start_result(startwith_data, include_results):
    ...
    for i in range(len(startwith_data)):
        ...
        for j in range(len(sentences)):
            ...
            with torch.no_grad():
                if args.mode != "ptuning":
                    outputs = start_model(**inputs, labels=label)
                else:
